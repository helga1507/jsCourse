...
function collectDOMStat(root) {
    ...
    function collectRecursive(root) {
        for (let i = 0; i < root.childNodes.length; i++) {
            let itemRoot = root.childNodes[i];

            if (itemRoot.nodeType === 3) {
                result.texts++;
            } else if (itemRoot.nodeType === 1) {
                result.tags[itemRoot.tagName] = (itemRoot.tagName in result.tags) ? result.tags[itemRoot.tagName] + 1 : 1;

                for (let item of itemRoot.classList) {
                    result.classes[item] = (item in result.classes) ? result.classes[item] + 1 : 1;
                }
            }

            if (itemRoot.childNodes && itemRoot.childNodes.length) {
                collectRecursive(itemRoot);
            }
        }
    }
