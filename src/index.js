...
function delegate(target, fn) {
    target.addEventListener('click', (e) => e.target.tagName === 'BUTTON' ? fn() : '');
}
...
function once(target, fn) {
    ...
    const newFunc = (target) => {
        if (x) {
            x = 0;
            fn();
            target.removeEventListener('click', newFunc);
        }
    };

    target.addEventListener('click', (e) => newFunc(e.currentTarget));
}
